...
def main(args=None):
    ...
    input_info = input('Please input the PDB code and chain index of the structure you initialized or will initialize your simulation with) e.g. (3pp0, A): ').replace(' ','').split(',')
    ...
    atoms = table.values
    chain_index = ord(str(pdb_chainid[1]).lower())-97 # translate a letter chain id into a number index (A->0, B->1 etc) 
...
#       * five ditances (a 5*2 array where each row contains indices of the two atoms for each dihedral) 
    ...
    for line in atoms:
        # for the soecified chain 
        if line[5] == chain_index:
            # dihedral 1: between aC and aE helices
            dih[0][0] = line[0] if line[3] == numbering[20] and line[1] == 'CA' else dih[0][0]
            dih[0][1] = line[0] if line[3] == numbering[28] and line[1] == 'CA' else dih[0][1]
            dih[0][2] = line[0] if line[3] == numbering[60] and line[1] == 'CA' else dih[0][2]
            dih[0][3] = line[0] if line[3] == numbering[62] and line[1] == 'CA' else dih[0][3]

            # dihedral 2 & 3: X-DFG Phi & Psi
            dih[1][0] = line[0] if line[3] == numbering[78] and line[1] == 'C' else dih[1][0]
            dih[1][1] = line[0] if line[3] == numbering[79] and line[1] == 'N' else dih[1][1]
            dih[1][2] = line[0] if line[3] == numbering[79] and line[1] == 'CA' else dih[1][2]
            dih[1][3] = line[0] if line[3] == numbering[79] and line[1] == 'C' else dih[1][3]
            dih[2][0] = dih[1][1]
            dih[2][1] = dih[1][2]
            dih[2][2] = dih[1][3]
            dih[2][3] = line[0] if line[3] == numbering[80] and line[1] == 'N' else dih[2][3]

            # dihedral 4 & 5: DFG-Asp Phi & Psi
            dih[3][0] = dih[1][3]
            dih[3][1] = dih[2][3]
            dih[3][2] = line[0] if line[3] == numbering[80] and line[1] == 'CA' else dih[3][2]
            dih[3][3] = line[0] if line[3] == numbering[80] and line[1] == 'C' else dih[3][3]
            dih[4][0] = dih[3][1]
            dih[4][1] = dih[3][2]
            dih[4][2] = dih[3][3]
            dih[4][3] = line[0] if line[3] == numbering[81] and line[1] == 'N' else dih[4][3]

            # dihedral 6 & 7: DFG-Phe Phi & Psi
            dih[5][0] = dih[3][3]
            dih[5][1] = dih[4][3]
            dih[5][2] = line[0] if line[3] == numbering[81] and line[1] == 'CA' else dih[5][2]
            dih[5][3] = line[0] if line[3] == numbering[81] and line[1] == 'C' else dih[5][3]
            dih[6][0] = dih[5][1]
            dih[6][1] = dih[5][2]
            dih[6][2] = dih[5][3]
            dih[6][3] = line[0] if line[3] == numbering[82] and line[1] == 'N' else dih[6][3]
        
            # dihedral 8: DFG-Phe Chi
            dih[7][0] = dih[5][1]
            dih[7][1] = dih[5][2]
            dih[7][2] = line[0] if line[3] == numbering[81] and line[1] == 'CB' else dih[7][2]
            dih[7][3] = line[0] if line[3] == numbering[81] and line[1] == 'CG' else dih[7][3]
        
            # distance 1 & 2: K-E salt bridge
            dis[0][0] = line[0] if line[3] == numbering[16] and line[1] == 'NZ' else dis[0][0]
            dis[0][1] = line[0] if line[3] == numbering[23] and line[1] == 'OE1' else dis[0][1]
            dis[1][0] = line[0] if line[3] == numbering[16] and line[1] == 'NZ' else dis[1][0]
            dis[1][1] = line[0] if line[3] == numbering[23] and line[1] == 'OE2' else dis[1][1]

            # distance 3 & 4: DFG conformation-related distances
            dis[2][0] = line[0] if line[3] == numbering[27] and line[1] == 'CA' else dis[2][0]
            dis[2][1] = line[0] if line[3] == numbering[81] and line[1] == 'CZ' else dis[2][1]
            dis[3][0] = line[0] if line[3] == numbering[16] and line[1] == 'CA' else dis[3][0]
            dis[3][1] = dis[2][1]
        
            # distance 5: FRET distance
            dis[4][0] = line[0] if line[3] == int(numbering[80]+10) and line[1] == 'CA' else dis[4][0]
            dis[4][1] = line[0] if line[3] == int(numbering[80]-20) and line[1] == 'CA' else dis[4][1] 
    ...
    user_input = str(input('Please specify the (full path to) the trajectory to analyze (trajectory,topology) accepted formats of trajectory include: ".arc", ".dcd", ".binpos", ".xtc", ".trr", ".hdf5", ".h5", ".ncdf", ".netcdf", ".nc", ".pdb.gz", ".pdb", ".lh5", ".crd", ".mdcrd", ".inpcrd", ".restrt", ".rst7", ".ncrst", ".lammpstrj", ".dtr", ".stk", ".gro", ".xyz.gz", ".xyz", ".tng", ".xml", ".mol2", ".hoomdxml"; otherwise type "pdb" if you want the current pdb structure to be analyzed): ')).replace(' ','').split(',')
    traj_top = tuple(user_input)
    if user_input == 'pdb':
        traj = md.load(str(pdb_chainid[0])+'.pdb')
    else:
        traj = md.load(str(traj_top[0]), top=str(traj_top[1]))
    print (str(traj_top[0]),str(traj_top[1]))
